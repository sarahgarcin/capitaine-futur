...
kirbytext::$tags['leftpic'] = array(
  ...
  'html' => function($tag) {
    ...
    if(kirby()->option('kirbytext.image.figure') or !empty($caption)) {
      ...
      $figure->attr('data-flou', $tag->attr('flou'));
      $figure->append($image);
      if(!empty($caption)) {
        $figure->append('<figcaption>' . html($caption) . '</figcaption>');
      }
      return '<div class="leftpic small-18 large-5">'.$figure.'</div>';
    } else {
      $class = trim($tag->attr('class') . ' ' . $tag->attr('imgclass'));
      return '<div class="leftpic small-18 large-5">'.$_link($_image($class)).'</div>';
    }
